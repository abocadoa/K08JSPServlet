...
public class LifeCycle extends HttpServlet{
	/*
	 서블릿 생성주기에서 최초로 호출되는 메소드로 어노테이션을 통해 생성한다.
	 따라서 메소드명은 개발자가 결정하면 된다.
	 init()메소드가 호출되기 전 전처리를 위해 주로 사용된다.
	 */
	@PostConstruct
	...
	/*
	 서블릿 객체 생성 후 딱 한번만 호출되는 메소드로, 보통 서블릿을 초기화하는 역할을 한다.
	 */
	@Override
	...
	ServletException, IOException {
		...
		/*
		 service()메소드에서 요청방식을 분석한 후 각 메소드를 호출할때 별도의 호출문장을
		 기술하지 않는다.
		 단지 아래 문장이면 된다.
		 */
