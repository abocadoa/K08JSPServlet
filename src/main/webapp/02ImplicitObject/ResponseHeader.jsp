...
만약 9시 이전의 시간으로 설정되면 어제날짜가 출력된다.*/
SimpleDateFormat s=new SimpleDateFormat("yyyy-MM-dd HH:mm");
long add_date=s.parse(request.getParameter("add_date")).getTime();
/* 숫자형식으로 전송된 값은 정수로 변한다. */
int add_int=Integer.parseInt(request.getParameter("add_int"));
/* 
문자형은 그대로 사용하면 된다.
서버로 전송된 파라미터는 모두 String타입으로만 받을 수 있다.
*/
...
<body>
	...
	동일한 값 8282가 출력된다.
	이것은 getHeader()메소드의 특성으로 처음 입력한 헤더값만 출력되게
	된다.
	*/
	%>
	...
	값이 정상적으로 출력된다.
	즉, add계열의 메소드는 헤더명을 동일하게 사용하더라도 헤더값은
	정상적으로 추가된다.
	*/
