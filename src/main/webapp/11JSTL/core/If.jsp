...
속성정리
 ...
 -->
	...
	<!-- 
	if(number%2==0)과 동일한 조건의 if문으로 해당 조건의 결과가 result에
	저장된다.
	...
	result4:${result4 } <br />
	<!-- test에는 앞뒤에 하나라도 공백이 들어가면 무조건 false를 반환하게
	된다.
	따라서 주의해야한다. -->
	...
	<h4>연습문제:if태그</h4>
	<!-- 
	아이디, 패스워드를 입력 후 submit버튼을 누르면 EL식을 통해 파라미터를 받은
	후 tjoeun/1234인 경우에는 'tjoeun님, 하이룽~'이라고 출력한다.
	...
	<%--<c:if test="${param.user eq 'tjoeun' and param.pass eq '1234' }"
		var="gojsp">
		tjoeun님, 하이룽~	
	</c:if>
	<c:if test="${not gojsp }">
	...
	<c:if test="${ not empty param.user }">
		...
		<!-- 
		아이디, 패스워드를 입력했을때만 일치여부를 판단한 후 결과를 변수에
		저장한다.
